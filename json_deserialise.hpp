...
public:
    ...
    const enum class AsType : uint8_t {
        NonTrivial = 0,
        LIMITED = 1,
        NULLABLE = 2,
        STRING = 4,
...
protected:
    ...
    DeserialisableBase(const QString& name, AsType _as) : as(_as), identifier(name) {}
    ...
    void deserialise(const QJsonObject& object) {
        ...
        if (complexity)
            for (auto i = 0; i < complexity; ++i) {
                ...
                if (deseriliser->anonymous || !object.contains(deseriliser->identifier))
                    throw std::ios_base::failure("JSON Structure Incompatible!");
...
public:
    ...
    Boolean(const QString& name, bool& source)
        : DeserialisableBase(name, AsType::BOOLEAN), value(source) {}
...
public:
    ...
    Integer(const QString& name, int& source)
        : DeserialisableBase(name, AsType::INTEGER), value(source) {}
    virtual void assign(const QJsonValue& data) override {
        if (data.isString())
...
public:
    ...
    Integer(const QString& name, unsigned& source)
        : DeserialisableBase(name, AsType::INTEGER), value(source) {}
    virtual void assign(const QJsonValue& data) override {
        if (data.isString())
...
public:
    ...
    Real(const QString& name, double& source)
        : DeserialisableBase(name, AsType::INTEGER), value(source) {}
    virtual void assign(const QJsonValue& data) override {
        if (data.isString())
...
public:
    ...
    String(const QString& name, Target& source)
        : DeserialisableBase(name, AsType::STRING), value(source) {}
    virtual void assign(const QJsonValue& data) override {
...
public:
    ...
    NullableString(const QString& name, Target& source)
        : DeserialisableBase(name, as), value(source) {}
    virtual void assign(const QJsonValue& data) override {
        if (!data.isString() && !data.isNull())
            throw std::ios_base::failure("Type Unmatch!");
        if (!data.isNull()) {
...
public:
    ...
    StringArray(const QString& name, Target& source)
        : DeserialisableBase(name, as), value(source) {}
...
public:
    constexpr static AsType as = AsType(unsigned(AsType::NULLABLE) | unsigned(AsType::STRING) |
                                        unsigned(AsType::ARRAY_LIKE));
    NullableStringArray(Target& source) : DeserialisableBase(as), value(source) {}
    NullableStringArray(const QString& name, Target& source)
        : DeserialisableBase(name, as), value(source) {}
    virtual void assign(const QJsonValue& data) override {
        if (!data.isArray() && !data.isNull())
            throw std::ios_base::failure("Type Unmatch!");
...
public:
    constexpr static AsType as =
        AsType(unsigned(AsType::STRING) | unsigned(AsType::ARRAY_LIKE) | unsigned(AsType::LIMITED));
    LimitedStringArray(Target& source) : DeserialisableBase(as), value(source) {}
    LimitedStringArray(const QString& name, Target& source)
        : DeserialisableBase(name, as), value(source) {}
    virtual void assign(const QJsonValue& data) override {
        if (!data.isArray() && !data.isNull())
            throw std::ios_base::failure("Type Unmatch!");
...
public:
    constexpr static AsType as = AsType(unsigned(AsType::NULLABLE) | unsigned(AsType::STRING) |
                                        unsigned(AsType::ARRAY_LIKE) | unsigned(AsType::LIMITED));
    LimitedNullableStringArray(Target& source) : DeserialisableBase(as), value(source) {}
    LimitedNullableStringArray(const QString& name, Target& source)
        : DeserialisableBase(name, as), value(source) {}
    virtual void assign(const QJsonValue& data) override {
        if (!data.isArray() && !data.isNull())
            throw std::ios_base::failure("Type Unmatch!");
...
public:
    ...
    ObjectArray(T& source, ObjectType* object_ptr, Info<Members>&&... members)
        : DeserialisableBase(as), value(source), offset{reinterpret_cast<size_t>(members.ptr)...},
          names{members.name...} {}
...
public:
    ...
    Array(const QString& name, Target& source)
        : DeserialisableBase(name, AsType::ARRAY_LIKE), value(source) {}
...
public:
    ...
    Nullable(const QString& name, T& source)
        : DeserialisableBase(name, AsType::NULLABLE), value(source) {}
    virtual void assign(const QJsonValue& data) override {
        if (data.isNull()) {
...
public:
    NonTrivial(Target& source)
        : DeserialisableBase(AsType::NonTrivial), value(reinterpret_cast<As&>(source)) {}
    NonTrivial(const QString& name, Target& source)
        : DeserialisableBase(name, AsType::NonTrivial), value(name, reinterpret_cast<As&>(source)) {
    }
    virtual void assign(const QJsonValue& data) override {
        value.assign(data);
    }
...
public:
    MapArray(Target& source)
        : DeserialisableBase( AsType(unsigned(AsType(AsType::ARRAY_LIKE)) | unsigned(AsType(AsType::OBJECT)))), value(source) {}
    virtual void assign(const QJsonValue& data) override {
        if (!data.isArray() && !data.isNull())
            throw std::ios_base::failure("Type Unmatch!");
        for (const auto& i : data.toArray()) {
...
public:
    MapArray(Target& source, QString key_name = QStringLiteral("key"))
        : DeserialisableBase(AsType(unsigned(AsType::ARRAY_LIKE) | unsigned(AsType::OBJECT))), value(source), key(key_name) {}
    MapArray(Target& source, QString key_name, QString val_json_name)
        : DeserialisableBase(AsType(unsigned(AsType::ARRAY_LIKE) | unsigned(AsType::OBJECT))), value(source), key(key_name), val_name(val_json_name) {}
...
public:
    Pair(Target& source, const QString& name1, const QString& name2)
        : DeserialisableBase(AsType::OBJECT), value(source), name{name1, name2} {}
    Pair(const QString& json_name, Target& source, const QString& name1, const QString& name2)
        : DeserialisableBase(json_name, AsType::OBJECT), value(source), name{name1, name2} {}
...
public:
    PairArray(Target& source, const QString& name1, const QString& name2)
        : DeserialisableBase(AsType::ARRAY_LIKE), value(source), name{name1, name2} {}
...
public:
    Extension(Convertor&& convertor, Target& source)
        : DeserialisableBase(AsType::NonTrivial), value(source), convertor(convertor), prototype(tmp) {}
    Extension(Convertor&& convertor, const QString& json_name, Target& source)
        : DeserialisableBase(json_name, AsType::STRING), value(source), convertor(convertor), prototype(tmp) {}
    ...
    virtual QJsonValue to_json() const override {
        if constexpr (Remove_Const_Of_C_Style<typename Convertor::Source>::value)
            tmp = const_cast<typename Remove_Const_Of_C_Style<typename Convertor::Source>::Type>(
                convertor.deconvertor(value));
        else
...
public:
    SerialiseOnlyExtension(Convertor&& convertor, const Source& source)
        : DeserialisableBase(AsType::NonTrivial), src(source), convertor(convertor), prototype(value) {}
    SerialiseOnlyExtension(Convertor&& convertor, const QString& json_name, const Source& source)
        : DeserialisableBase(json_name, AsType::STRING), src(source), convertor(convertor), prototype(value) {}
...
struct JSONWrap : public DeserialisableBase {
    ...
    JSONWrap(const QString& name, QJsonValue& src)
        : DeserialisableBase(name, AsType::NonTrivial), value(src) {}
...
#define register_map_with_key(container_type, key_type, value_type, key_name)                      \
    namespace JsonDeserialise {                                                                    \
        constexpr char key_type##_##map_type##_##key_json_name[] = key_name;                       \
        template <>                                                                                \
        struct Deserialisable<container_type<key_type, value_type>> {                              \
            using Type = MapArray<container_type<key_type, value_type>, key_type, value_type,      \
                                  key_type##_##map_type##_##key_json_name>;                        \
        };                                                                                         \
    }
